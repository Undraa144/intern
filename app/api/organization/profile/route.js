...
function getOrganizationProfile(token) {
  return fetch(`${API_BASE}/api/organization/profile`, {
    method: "GET",
    headers: {
...
function updateOrganizationProfile(token, body) {
  return fetch(`${API_BASE}/api/organization/profile`, {
    method: "PUT",
    headers: {
...
export async function GET() {
  ...
  try {
    let response = await getOrganizationProfile(token);
    ...
    console.error("Organization profile request failed:", error);
    return Response.json(
      { message: "Байгууллагын профайлыг серверээс авч чадсангүй." },
      { status: 502 }
    );
  }
}
...
export async function PUT(request) {
  ...
  try {
    ...
    let response = await updateOrganizationProfile(token, body);
    ...
    return Response.json(
      { message: "Байгууллагын профайлыг серверт хадгалж чадсангүй." },
      { status: 502 }
    );
  }
}

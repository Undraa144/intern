...
function addStudent(token) {
  return fetch(`${API_BASE}/api/teacher/students`, {
    method: "PUT",
...
export async function PUT() {
  ...
  try {
    let response = await addStudent(token);
    ...
    console.error("Adding teacher student failed:", error);
    return Response.json(
      { message: "Оюутныг серверт нэмж чадсангүй." },
      { status: 502 }
    );
  }
}
